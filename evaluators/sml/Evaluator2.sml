...
*)
       ...
       then let val v = T.dest_variable t
	    in case T.lookup_clos env v of
		 ...
		 | NONE => raise Fail ("evaluating:variable(" ^ T.toStringTerm t ^ ")")
	    end
...
fun CEvaluator2' call_by_need term state =
	...
	and Eval (t, env) state =
	    ...
	    then Eval (T.dest_ct t) state
		     ...
		     val _ = if n = ~568 then
				 (print ("\n" ^ T.toStringTerm (T.mk_nuprl_ref_term opr ((clos_refs c) @ np)) ^ "\n");
				  raise Fail "~568")
			     else ()
		 (* ************************************** *)*)
		 in let val ((nt,e',ev),s3) = ClosNextStepEval2 cls lib t c np env s2
		    ...
		    end handle _ => ((T.mk_nuprl_ref_term opr ((clos_refs c) @ np), env), s2)
		 end
...
fun Evaluator2 call_by_need cls lib steps term =
    let (*val _ = print "[starting evaluation]\n"*)
	val ((t',e'),(n,_)) = CEvaluator2' call_by_need term (steps,(lib,cls))
