...
(nvunion-deq "PRODUCT" (nvlist "PRODUCT") (nvproduct-deq "INT" (nvlist "ATOM") (nvint-deq) (nvlist-deq (nvatom-deq))) (nvlist-deq (nvproduct-deq "INT" nvOp (nvint-deq) nveq_Op))))
...
(let ((pair (cons (funcall nvs0) (nvcond-msg-body nvhdr (funcall nvm)))))
(let ((nvs1 (car pair))
(nvb (cdr pair)))
(cons (funcall (funcall nvmk-hdf)
(lambda () nvs1)) nvb)))))))))
(lambda () nil))))
...
(declare (ignore nvmf))
(nvbase-class-program (cons "rsc4" (cons "vote" nil))))
...
(nvt (cdr pair)))
(cons (funcall nvf
(lambda () nvh)) (funcall (funcall nvlist_ind)
(lambda () nvt)))))
(if (not nvv)
nil
(error "bottom")))))))
...
(defun nvappend (nvas nvbs)
  (if (consp nvas)
      (let ((nvh (car nvas))
	    (nvt (cdr nvas)))
	(cons nvh (nvappend nvt nvbs)))
      (if (not nvas)
	  nvbs
	  (error "bottom"))))

(defun nvappend1 (nvas nvbs)
(funcall (fix "nvlist_ind" (lambda (nvlist_ind)
(lambda (nvL)
(let ((nvv (funcall nvL)))
(if (consp nvv)
...
(let ((nvh (car pair))
(nvt (cdr pair)))
(cons nvh (funcall (funcall nvlist_ind)
(lambda () nvt)))))
(if (not nvv)
nvbs
(error "bottom")))))))
...
(nvreduce (lambda (nvl)
(lambda (nvl_)
(nvappend (funcall nvl) (funcall nvl_)))) nil nvll))
...
(funcall (fix "nvfrom-upto" (lambda (nvfrom-upto)
(lambda (nvn)
(if (inl-p (nvlt_int (funcall nvn) nvm))
(cons (funcall nvn) (let ((nvn_ (+ (funcall nvn) 1)))
...
(cons nvn (nvappend nvmissing (nvfrom-upto (+ nvmax 1) nvn)))
(cons nvmax (nvlist-diff (nvint-deq) nvmissing (cons nvn nil)))))))))))
...
(lambda () (cons nvn nvc)))
(lambda () (cons nvmax nvmissing)))
(cons nvmax nvmissing))))))))))
...
nvs
nvb)))
(cons (cons nvX_ nvs_) nvs)))))))))
(let ((nvs1 (car pair))
(nvb (cdr pair)))
(cons (funcall (funcall nvmk-hdf)
(lambda () nvs1)) nvb)))))))))
(lambda () (cons nvX nvbs))))
...
(lambda () (cons 0 nil))) (nvrsc4_update_replica nvCmd) (nvrsc4_Proposal-program nvCmd nvnotify nvpropose nvmf)))
...
(nvmk-msg-interface (funcall nvl) (nvmake-Msg (cons "rsc4" (cons "vote" nil)) (funcall nvz)))) (funcall nvlocs)))))
...
(lambda (nvl)
(lambda (nvz)
(nvmk-msg-interface (funcall nvl) (nvmake-Msg (cons "rsc4" (cons "decided" nil)) (funcall nvz))))))
...
(lambda (nvl)
(lambda (nvz)
(nvmk-msg-interface (funcall nvl) (nvmake-Msg (cons "rsc4" (cons "retry" nil)) (funcall nvz))))))
...
(let ((pair (nvposs-maj nvcmdeq (cons nvc nvcmds) nvc)))
(let ((nvk (car pair))
(nvx (cdr pair)))
...
(lambda () (funcall nvloc)))
(lambda () (cons (cons nvn (+ nvi 1)) nvx)))))))
nil)))))))))))))
...
(let ((nvcmds (car pair))
(nvz (cdr pair)))
(cons (cons nvc nvcmds) (cons nvsender nvz)))))))))))
...
(lambda (nvni)
(nvmemory-class1-program (funcall (nvrsc4_init)
(lambda () (cons nil nil))) (funcall (nvrsc4_add_to_quorum nvCmd)
...
(lambda () nvlocs))
(lambda () (cons (cons nvni nvc) (funcall nvloc)))))) (nvonce-class-program (funcall (nvrsc4_Quorum-program nvCmd nvcmdeq nvcoeff nvflrs nvnotify nvpropose nvmf)
(lambda () nvni))))))))
...
(declare (ignore nvpropose))
(declare (ignore nvmf))
(nvbase-class-program (cons "rsc4" (cons "decided" nil))))
...
(nvsingle-bag (cons (cons nvm nvi) nvc))
nil))))))))))
...
(declare (ignore nvpropose))
(declare (ignore nvmf))
(nvbase-class-program (cons "rsc4" (cons "retry" nil))))
...
(let ((nvY_ (car pair))
(nvc (cdr pair)))
(cons (cons (if (inl-p (nvbag-null nvc))
nvX_
(nvhdf-halt)) nvY_) nvb)))))))))
(let ((nvs1 (car pair))
(nvb (cdr pair)))
...
(lambda () (cons (cons nvn 0) nvc))) (funcall (nvrsc4_Notify-program nvCmd nvclients nvnotify nvpropose nvmf)
...
(if (string= (funcall nva) (funcall nvb))
(make-inl :val nil)
(make-inr :val nil)))) (funcall nvi) nvlocs))
(funcall nvpr
(lambda () (funcall nvi)))
(nvhdf-halt))))
...
(nvrsc4_main-program "UNION" nvreps (nvaneris_control_tagged_cmd_deq nvOp nveq_Op) 2 nvflrs nvlocs (cons "decision" nil) (cons "rsc4" (cons "propose" nil)) nvmf))
...
(lambda (nvl)
(lambda (nvz)
(nvmk-msg-interface (funcall nvl) (nvmake-Msg (cons "aneris_pax_v1" (cons "ping" nil)) (funcall nvz))))))
...
(lambda (nvl)
(lambda (nvz)
(nvmake-msg-interface (funcall nvn) (funcall nvl) (nvmake-Msg (cons "aneris_pax_v1" (cons "react" nil)) (funcall nvz)))))))
...
(lambda (nvl)
(lambda (nvz)
(nvmk-msg-interface (funcall nvl) (nvmake-Msg (cons "aneris_pax_v1" (cons "start" nil)) (funcall nvz))))))
...
(declare (ignore nvpropose))
(declare (ignore nvmf))
(nvbase-class-program (cons "aneris_pax_v1" (cons "react" nil))))
...
(declare (ignore nvpropose))
(declare (ignore nvmf))
(nvbase-class-program (cons "aneris_pax_v1" (cons "pong" nil))))
...
(lambda () 10000))
(lambda () (funcall nvldr)))
(lambda () nil)))))) (funcall (nvaneris_pax_v1_MonitorReact-program nvCmd nvdecision nvpropose nvmf)
(lambda () (funcall nvloc))))))
...
(lambda (nvz)
(nvbag-map (lambda (nvl)
(nvmk-msg-interface (funcall nvl) (nvmake-Msg (cons "aneris_pax_v1" (cons "p1a" nil)) (funcall nvz)))) (funcall nvlocs)))))
...
(lambda () nvaccpts))
(lambda () (cons (funcall nvldr) (funcall nvbn))))))))
...
(lambda (nvl)
(lambda (nvz)
(nvmk-msg-interface (funcall nvl) (nvmake-Msg (cons "aneris_pax_v1" (cons "adopted" nil)) (funcall nvz))))))
...
(lambda (nvl)
(lambda (nvz)
(nvmk-msg-interface (funcall nvl) (nvmake-Msg (cons "aneris_pax_v1" (cons "preempted" nil)) (funcall nvz))))))
...
(lambda () (funcall nvldr)))
(lambda () (cons (funcall nvbnum) nvpvalues))))
nil)
...
(declare (ignore nvpropose))
(declare (ignore nvmf))
(nvbase-class-program (cons "aneris_pax_v1" (cons "p1b" nil))))
...
(cons nvaccpts (nvaneris_pax_v1_init_pvalues nvCmd)))
...
(cons (funcall (funcall nvf
(lambda () nvx))
(lambda () nvh)) nvt)
(if (inl-p (nvlt_int 0 nvtst))
(cons nvx (cons nvh nvt))
(cons nvh (funcall (funcall nvlist_ind)
(lambda () nvt))))))))
(if (not nvv)
(cons nvx nil)
...
(if (string= (funcall nva) (funcall nvb))
(make-inl :val nil)
(make-inr :val nil)))) nvwaitfor nvw)))
(let ((nvpvalues_ (funcall (funcall (nvaneris_pax_v1_add_new_pvalues nvCmd nvldrs_uid)
(lambda () nvpvalues))
(lambda () nvz_))))
(cons nvwaitfor_ nvpvalues_)))
(cons nvwaitfor nvpvalues)))))))))))))))
...
(lambda (nvn)
(lambda (nvloc)
(make-inl :val (cons (funcall nvn) (funcall nvloc))))))
...
(declare (ignore nvpropose))
(declare (ignore nvmf))
(nvbase-class-program (cons "aneris_pax_v1" (cons "start" nil))))
...
(lambda (nvbn)
(lambda (nvprp)
(cons (funcall nvbn) (funcall nvprp)))))
...
(lambda (nvloc)
(cons (funcall (nvaneris_pax_v1_init_ballot_num)
(lambda () (funcall nvloc))) (cons (nvaneris_pax_v1_init_active) (cons (nvaneris_pax_v1_init_proposals nvCmd) (nvaneris_pax_v1_init_decided))))))

(defun nvaneris_pax_v1_mk_prp (nvCmd)
(declare (ignore nvCmd))
(lambda (nvslt)
(lambda (nvcmd)
(cons (funcall nvslt) (funcall nvcmd)))))
...
(lambda () nvslt))
(lambda () nvcmd)))))
(cons nvw (cons nvx (cons nvproposals_ nvz_))))))))))))))))
...
(let ((nvbn (car pair))
(nvprp (cdr pair)))
(cons nvbn (funcall (nvaneris_pax_v1_dest_prp nvCmd)
(lambda () nvprp)))))))
...
(lambda () nvz_))
(lambda () nvpvals))))
(cons nvw (cons (nvbtrue) (cons nvproposals_ nvz_))))
(cons nvw (cons nvx (cons nvy nvz_))))))))))))))))
...
(declare (ignore nvpropose))
(declare (ignore nvmf))
(nvbase-class-program (cons "aneris_pax_v1" (cons "adopted" nil))))
...
(lambda () nvw))
(lambda () (funcall nvbnum)))))
(cons (funcall (funcall (nvaneris_pax_v1_upd_bnum)
(lambda () (funcall nvbnum)))
(lambda () (funcall nvldr))) (cons (nvbfalse) (cons nvy nvz_)))
(cons nvw (cons nvx (cons nvy nvz_))))))))))))))
...
(declare (ignore nvpropose))
(declare (ignore nvmf))
(nvbase-class-program (cons "aneris_pax_v1" (cons "preempted" nil))))
...
(cons (funcall nvslt) (nvappend nvmissing (nvfrom-upto (+ nvmax 1) (funcall nvslt))))
(if (inl-p (nveq_int (funcall nvslt) nvmax))
(cons nvmax nvmissing)
(cons nvmax (funcall (funcall (nvaneris_pax_v1_remove_from_missing)
(lambda () (funcall nvslt)))
(lambda () nvmissing))))))))))
...
(lambda () (funcall nvslt)))
(lambda () nvz_))))
(cons nvw (cons nvx (cons nvproposals_ nvdecided_)))))))))))))))
...
(declare (ignore nvpropose))
(declare (ignore nvmf))
(nvbase-class-program (cons "aneris_pax_v1" (cons "decided" nil))))
...
(lambda (nvz)
(nvbag-map (lambda (nvl)
(nvmk-msg-interface (funcall nvl) (nvmake-Msg (cons "aneris_pax_v1" (cons "p2a" nil)) (funcall nvz)))) (funcall nvlocs)))))
...
(lambda () nvaccpts))
(lambda () (cons (funcall nvldr) (funcall nvpvalue))))))))
...
(lambda (nvz)
(nvbag-map (lambda (nvl)
(nvmk-msg-interface (funcall nvl) (nvmake-Msg (cons "aneris_pax_v1" (cons "decided" nil)) (funcall nvz)))) (funcall nvlocs)))))
...
(declare (ignore nvpropose))
(declare (ignore nvmf))
(nvbase-class-program (cons "aneris_pax_v1" (cons "p2b" nil))))
...
(if (string= (funcall nva) (funcall nvb))
(make-inl :val nil)
(make-inr :val nil)))) (funcall nvwaitfor) nvw)
(funcall nvwaitfor))))))))))))))
...
(lambda (nvl)
(lambda (nvz)
(nvmk-msg-interface (funcall nvl) (nvmake-Msg (cons "aneris_pax_v1" (cons "pong" nil)) (funcall nvz))))))
...
(declare (ignore nvpropose))
(declare (ignore nvmf))
(nvbase-class-program (cons "aneris_pax_v1" (cons "ping" nil))))
...
(lambda () (funcall nvloc)))
(lambda () nil))))) (nvaneris_pax_v1_ping_base-program nvCmd nvdecision nvpropose nvmf))))
...
(lambda (nvl)
(lambda (nvz)
(nvmk-msg-interface (funcall nvl) (nvmake-Msg (cons "aneris_pax_v1" (cons "p1b" nil)) (funcall nvz))))))
...
(declare (ignore nvpropose))
(declare (ignore nvmf))
(nvbase-class-program (cons "aneris_pax_v1" (cons "p1a" nil))))
...
(cons (nvaneris_pax_v1_dummy_ballot) (nvaneris_pax_v1_init_accepted nvCmd)))
...
(declare (ignore nvloc))
(let ((pair (funcall nvz)))
(let ((nvballot_num (car pair))
(nvaccepted (cdr pair)))
(cons (funcall (funcall (nvaneris_pax_v1_max_bnum nvldrs_uid)
(lambda () nvbnum))
(lambda () nvballot_num)) nvaccepted)))))))))
...
(lambda () nvpvalue))
nvaccepted)))
(cons nvballot_num_ nvaccepted_))))))))))))
...
(declare (ignore nvpropose))
(declare (ignore nvmf))
(nvbase-class-program (cons "aneris_pax_v1" (cons "p2a" nil))))
...
(lambda () nvldr))
(lambda () (cons (funcall nvloc) (cons nvbn (cons nvbnum nvpvals)))))))))))))))
...
(lambda (nvl)
(lambda (nvz)
(nvmk-msg-interface (funcall nvl) (nvmake-Msg (cons "aneris_pax_v1" (cons "p2b" nil)) (funcall nvz))))))
...
(lambda () nvldr))
(lambda () (cons (funcall nvloc) (cons nvx (cons nvy nvbnum)))))))))))))))))))
...
(declare (ignore nvOp))
(nvaneris_pax_v1_main-program "UNION" nvaccpts (cons "decision" nil) nvldrs nvldrs_uid (cons "pv11_p1" (cons "propose" nil)) nvreps nvmf))
...
(cons (nvaneris_batch_init_slot_num) (cons (nvaneris_batch_init_active) (cons (nvaneris_batch_init_atimer) (cons (nvaneris_batch_init_proposals nvOp) (cons (nvaneris_batch_init_decisions) (cons (nvaneris_batch_init_todo nvOp) (nvaneris_batch_init_protocol))))))))
...
(lambda () (funcall nvcmdt)))
(lambda () nve)))
(cons (cons nva (cons nvb (cons nvc (cons nvd (cons nve (cons nvf nvg)))))) nil)
...
(if (inl-p nvc)
(cons nil nvc)
(cons (nvsingle-bag (funcall (funcall (funcall (nvaneris_batch_timer__send nvOp nvbcast nvdecision nvpax_propose nvresponse nvswap nvtimer nvtt_propose nvmf)
(lambda () nvtimeout))
(lambda () (funcall nvloc)))
(lambda () nil))) (nvbtrue))))))
(let ((nvmsgs (car pair))
(nvatimer_ (cdr pair)))
(cons (cons nva (cons nvb (cons nvatimer_ (cons nvproposals_ (cons nve (cons nvf nvg)))))) nvmsgs))))))))))))))))))))))
...
(lambda () nvt)) (cons (let ((dec nvh))
(if (inl-p dec)
(error "bottom")
(if (inr-p dec)
(let ((nvz (inr-val dec)))
nvz)
(error "decide:an_injection_should_be_a_list_with_header_inl_or_inr")))) nil)))))
(if (not nvv)
nil
(error "bottom")))))))
(lambda () (funcall nvproposals)))))
...
(lambda () (cons nva nvbatch))))
nil)))))
(cons (cons nva (cons (nvbtrue) (cons nvc (cons nvd (cons nve (cons nvf nvg)))))) nvmsgs)))))))))))))))))
...
(let ((nvf (car pair))
(nvg (cdr pair)))
(cons (cons nva (cons nvb (cons (nvbfalse) (cons nvd (cons nve (cons nvf nvg)))))) nvmsgs)))))))))))))))))))
...
(lambda (nvprps2)
(funcall nvprps2))) (cons (funcall nvslt) (funcall nvcmdts)) (funcall nvtodo)))))))))
...
(let ((nvs_ (car pair))
(nvb_ (cdr pair)))
(cons nvs_ (nvbag-append nvb nvb_)))))))
(lambda () nvh))))
(lambda () nvt))))
(if (not nvv)
(funcall nvy)
(error "bottom"))))))))
(lambda () (cons (funcall nvinit) nil)))
...
(let ((pair nvz))
(let ((nvcid (car pair))
(nvnew_protocol (cdr pair)))
(declare (ignore nvcid))
nvnew_protocol)))
(if (inr-p dec)
(funcall nvprotocol)
(error "decide:an_injection_should_be_a_list_with_header_inl_or_inr"))))))
(cons nvprotocol_ (funcall (funcall (nvaneris_batch_response_broadcast nvOp nvbcast nvdecision nvpax_propose nvresponse nvswap nvtimer nvtt_propose nvmf)
(lambda () nvclients))
(lambda () (cons (funcall nvslot_num) (funcall nvcmdts))))))))))
...
(cons nvcid (nvappend nvmissing (nvfrom-upto (+ nvmax 1) nvcid)))
(if (inl-p (nveq_int nvcid nvmax))
(cons nvmax nvmissing)
(cons nvmax (funcall (funcall (nvaneris_batch_remove_from_missing)
(lambda () nvcid))
(lambda () nvmissing))))))))))))
...
(lambda () (funcall nvcmdts)))
(lambda () nve)))
(cons (cons nva (cons nvb (cons nvc (cons nvd (cons nve (cons nvtodo_ nvg)))))) nil)
...
(lambda () nva))
(lambda () nvg))))
(let ((nvprotocol_ (car pair))
(nvmsgs (cdr pair)))
(let ((nvdecisions_ (funcall (funcall (funcall (nvaneris_batch_add_decisions nvOp)
(lambda () nva))
(lambda () (funcall nvcmdts)))
(lambda () nve))))
(cons (cons (+ nva 1) (cons (nvbfalse) (cons nvc (cons nvd (cons nvdecisions_ (cons nvtodo_ nvprotocol_)))))) nvmsgs)))))))))))))))))))))
...
(lambda () nvcmdts))
(lambda () (cons nva (cons nvb (cons nvc (cons nvd (cons nve (cons nvf nvg))))))))
(cons (cons nva (cons nvb (cons nvc (cons nvd (cons nve (cons nvf nvg)))))) nil)))))))))))))))))))
...
(lambda () (cons nva (cons nvb (cons nvc (cons nvproposals_ (cons nve (cons nvtodo_ nvg))))))))
(lambda () nvtodo_))))
(let ((nvz (car pair))
(nvmsgs1 (cdr pair)))
(let ((pair nvz))
...
(let ((pair (funcall (nvaneris_batch_propose nvOp nvbcast nvdecision nvpax_procs nvpax_propose nvresponse nvswap nvtimer nvtt_procs nvtt_propose nvmf)
(lambda () (cons nva (cons nvb (cons nvc (cons nvd (cons nve (cons nvf nvg))))))))))
(let ((nvstate (car pair))
(nvmsgs2 (cdr pair)))
(cons nvstate (nvbag-append nvmsgs1 nvmsgs2))))))))))))))))))))))))))))))))))))))
...
(declare (ignore nvz))
(cons (nvaneris_batch_init_state nvOp) nil)) (funcall (nvaneris_batch_out_tr nvOp nvbcast nvdecision nvpax_propose nvresponse nvswap nvtimer nvtt_propose nvmf)
...
(nvaneris_batch_main-program nvOp (cons "bcast" nil) nvclients (cons "decision" nil) nvldrs (cons "pv11_p1" (cons "propose" nil)) nvreps (cons "response" nil) (cons "swap" nil) nvtimeout (cons "timer" nil) nvlocs (cons "rsc4" (cons "propose" nil)) nvmf))
