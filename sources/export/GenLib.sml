...
fun get_nb_args_of_type m opid =
	...
	List.length args
      | SOME [] => raise Fail ("get_type_signature:SOME:nil:" ^ opid)
      | SOME ((obid, lhs, rhs, []) :: xs) =>
	raise Fail ("get_type_signature:SOME:nil_wf:" ^ opid)
      | SOME ((obid, lhs, rhs, ((wf, NONE) :: wfs)) :: xs) =>
	raise Fail ("get_type_signature:SOME:wf_NONE:" ^ opid)
      | NONE => raise Fail ("get_type_signature:NONE")
			 ...
			 case MAP.find (m, id) of
			     ...
			     then ()
					  ...
					  (*app (fn st => TextIO.output (stout, st))
						lst*)
(*					      val _ = if id = "update-oarcast-deliver"
						      then print ("xxx:" ^ id ^ ":" ^ Int.toString (length lst) ^ "\n")
						      else ()*)
					  in case lst of
						 [] => ()
...
fun nuprlType2Ity force m sub bound (N.TERM ((("variable", tag), [(v, "v")]), [])) =
  ...
  | nuprlType2Ity force m sub bound (N.TERM ((("nat", tag), []), [])) =
    (case sub of
	 ...
	 SOME (E.mk_type_nat D.dummy_label)
       | NEG =>
	 if force
	 then (*SOME (E.mk_type_int D.dummy_label)*)
	     SOME (E.mk_type_nat D.dummy_label)
	 else SOME (E.mk_type_nat D.dummy_label))
  ...
  | nuprlType2Ity force m sub bound (N.TERM (((opid, tag), params), bterms)) =
    ...
    then let val itys   =
	 ...
	 in if List.length itys = List.length bterms
	       andalso
	       (List.length itys = get_nb_args_of_type m opid) handle _ => false
...
(*	val _      = print (if force then "force\n" else "no force\n")*)
