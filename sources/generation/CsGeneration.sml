...
fun sanityCheckOverloading tyvarseq tyseqset label =
	...
	fun getTyCon (A.N {kind = (A.TYPE, A.TYPE_TYCON), label, value, regions, parents, children = [tn, ts]}) =
	  ...
	  | getTyCon (A.N {kind = (A.TYPE, A.TYPE_SPSET), label, value, regions, parents, children = [id]}) =
	    raise Fail "TYPE_SPSET:not_implemented"
...
and gen_id_header (A.N {kind = (A.ID, A.ID_VID), label, value, regions, parents, children = []}) kind =
	...
	val sign  = value ^ "'sign"
	val bsign = value ^ "'bsign"
	val verif = value ^ "'verify"
	fun mk_send_envs name =
	    let val tv1  = E.nextItyvar ()
		val env1 = E.mk_env_depbin (E.BINDVID (name, label, E.mk_new_vscheme tv1)) label
	...
	fun mk_delayed_broadcast_envs name =
	    let val tv1  = E.nextItyvar ()
		val env1 = E.mk_env_depbin (E.BINDVID (name, label, E.mk_new_vscheme tv1)) label
		val bag  = E.mk_type_bag (E.mk_type_instr label) label
		val locs = E.mk_type_bag (E.mk_type_loc label) label
	...
	val envB = (* base class *) mk_base_envs base
	val envV = (* verify class *) mk_base_envs verif
	val envS = (* send/broadcast functions *)
	    if kind = "internal" orelse kind = "output"
	    then let (* send *)
		    val envs1 = mk_send_envs send
		    (* nsend *)
		    val envs2 = mk_delayed_send_envs nsend
		    (* broadcast *)
		    val envs3 = mk_broadcast_envs cast
		    (* nbroadcast *)
		    val envs4 = mk_delayed_broadcast_envs ncast
		    (* sign *)
		    val envs5 = mk_send_envs sign
		    (* bsign *)
		    val envs6 = mk_broadcast_envs bsign
		in envs1 @ envs2 @ envs3 @ envs4 @ envs5 @ envs6
		end
	    else if kind = "input"
	    then []
	    else raise Fail "gen_id_header"
	val envs = envB @ envV @ envS
	val env  = let val enverr = E.ENVDEP (E.ENVERR (value, E.REBOUND), D.mk_dep label)
		   in if List.exists (fn x => x = value) A.constructors
...
and gen_atexp (A.N {kind = (A.ATEXP, A.ATEXP_ID), label, value, regions, parents, children = [id]}) =
  ...
  | gen_atexp (A.N {kind = (A.ATEXP, A.ATEXP_LOCAL), label, value, regions, parents, children = [exp1,exp2,exp3]}) =
	...
	val tv'  = E.nextItyvar ()
	val cls  = E.mk_type_class (E.mk_tyvar tv') label
	val nam  = E.mk_type_list (E.mk_type_atom label) label
	val env4 = E.mk_env_depcst (E.CSITY (E.mk_tyvar tv1, cls)) label
	val env5 = E.mk_env_depcst (E.CSITY (E.mk_tyvar tv2, loc)) label
	val env6 = E.mk_env_depcst (E.CSITY (E.mk_tyvar tv3, nam)) label
	val env7 = E.mk_env_depcst (E.CSITY (E.mk_tyvar tv,  cls)) label
	val envl = E.mk_env_deplab (tv', E.OTH) label
	val env  = E.list2env [env1, env2, env3, env4, env5, env6, env7, envl]
    in (tv, env)
    end
...
and gen_exp (A.N {kind = (A.EXP, A.EXP_OR), label, value, regions, parents, children = [exp1, exp2]}) =
  ...
  | gen_exp (A.N {kind = (A.EXP, A.EXP_N2I), label, value, regions, parents, children = [atexp]}) =
    let val (tv, env) = gen_atexp atexp
	val tv'  = E.nextItyvar ()
	val env1 = E.mk_env_depcst (E.CSITY (E.mk_tyvar tv,  E.mk_type_nat label)) label
	val env2 = E.mk_env_depcst (E.CSITY (E.mk_tyvar tv', E.mk_type_int label)) label
	val env3 = E.ENVAPP (env, E.ENVAPP (env1, env2))
    in (tv', env3)
    end
...
and gen_dec defs (A.N {kind = (A.DEC, A.DEC_LET), label, value, regions, parents, children = [bind]}) =
  ...
  | gen_dec defs (A.N {kind = (A.DEC, A.DEC_SVCLASS), label, value, regions, parents, children = [bind]}) =
    let val (env1, env2) = gen_bind true bind
	val ev   = E.nextEnvvar ()
	val enve = bindExplicitTyVars [bind] E.EXPLICIT
	val env3 = E.mk_env_loc (enve, E.ENVPOL (E.ENVAPP (env2, env1), E.TOP))
