...
;;;;; TODO:
;;
;; 1) Fix boot-up so that the public keys come from the configuration
;; file, for example.
;;
;; 2) Update set-handlers so that it does the verify.  Done, but now
;; verify-test gives random results...
;;
...
;;;;;; LOAD THE LEXER, PARSER GENERATORS, ...
...
;;    o then: (ql:quickload "local-time")
;;    o then: (ql:quickload "cl-store")
;;    o then: (ql:add-to-init-file)
;;
;; Only do the ql:quickload's if quicklisp is already installed.
;;
...
;; true to get get time information
(defvar timedbg nil)
...
(defun parse-eml-bag-members (exp parse)
  ...
  (multiple-value-bind (match strings)
      (ppcre:scan-to-strings "[ ]*([^;]+);(.*)"
			     exp)
    (if (string= match exp)
	(cons (funcall parse (aref strings 0))
	      (parse-eml-bag-members (aref strings 1) parse))
	(if (ppcre:split "\\s+" exp)
	    (cons (funcall parse exp) nil)
	    nil
	    )
	)
    )
...
(defun product-deq (eq1 eq2)
  (lambda (p1)
    (lambda (p2)
	  ...
	  (band (funcall (funcall eq1 (car p1)) (car p2))
		(funcall (funcall eq2 (cdr p1)) (cdr p2))
		)
...
(eval-when (:compile-toplevel :load-toplevel :execute)
  ...
  (defun parse-explist-1 (exp)
    (lambda (lst) (cons (funcall exp lst) nil))
    )

  (defun parse-explist-2 (exp semicolon explist)
    (declare (ignore semicolon))
    (lambda (lst) (cons (funcall exp lst) (funcall explist lst)))
    )
  ...
  (defun parse-msg-0 (id colon lparen rparen)
    ...
    (make-intransit :id id
		    :header ""
		    :type ""
		    :expression (lambda (lst) (declare (ignore lst)) nil))
    )
...
;; Here we use SHA1.  However attacks have been found on SHA1.  We
;; should switch to something else.  Unfortunately, it's currently the
;; only solution available that is compatible with DSA (i.e., produces
;; hashes of size 20).
(defun mk-digest (obj)
  (ironclad:produce-digest
   (ironclad:update-digest
    (ironclad:make-digest :SHA1)
    (sb-ext:string-to-octets (write-to-string obj)))))

;; Uses DSA

;; Uses private-key 'prkey' to sign message
(defun mk-sig (loc digest)
  ;;(declare (ignore loc))
  ;;(print-eml-loc loc (format nil "signing with: ~A" prkey))
  ;; returns a signature
  (handler-case (ironclad:sign-message prkey digest)
    (error
	(c)
      (print-eml-loc loc (format nil "signing failed with: ~A" c))
      (print-eml-loc loc "re-trying")
      (mk-sig loc digest)
      ))
  )

(defun get-sig-rs (sig)
  (cons
   (ironclad:dsa-signature-r sig)
   (ironclad:dsa-signature-s sig)))

;; sig is (r . s)
(defun mk-dsa-sig (sig)
  (ironclad:make-dsa-signature (car sig) (cdr sig)))

;; k is (p . q . g . y)
(defun mk-dsa-public-key (k)
...
;; Let L=1024 and N=160

;; q is a N-bit prime
(defvar dsa-q-test 11)
(defvar dsa-p-test 23)
(defvar dsa-g-test 4) ;; h = 2
(defvar dsa-y-test 18)
(defvar dsa-x-test 3)
...
(defun sign-test1 ()
  (ironclad:sign-message
   ...
   (ironclad:produce-digest
    (ironclad:update-digest
     digest-test
     (flexi-streams:string-to-octets "xxx")))))

(defun sign-test2 (obj)
  (ironclad:sign-message
   prkey-test
   (ironclad:produce-digest
    (ironclad:update-digest
	 ...
	 (s :element-type '(unsigned-byte 8))
       (cl-store:store obj s))))))

(defun sign-test3 (obj)
  (ironclad:sign-message
   prkey-test
   (ironclad:produce-digest
    (ironclad:update-digest
...
(defun sign-test ()
  (ironclad:sign-message
   prkey-test
   (flexi-streams:string-to-octets "xyzuisdadsfdfgljsdhss")
   :end 20))
;; use (ironclad:dsa-signature-r (sign-test)) and (ironclad:dsa-signature-s (sign-test))
;; to extract the info from the signature
...
(defun verify-test (obj)
  (ironclad:verify-signature
   pbkey-test
   (mk-digest obj)
   (ironclad:sign-message prkey-test (mk-digest obj))))
...
;; ----
;; -- gets the locations from the list of locations that have an ip/port
...
(defun getms ()
  (let* ((n  (local-time:now))
	 (ns (local-time:nsec-of n))
	 (s  (local-time:sec-of n)))
    (+ (* s 1000000000) ns))
  )

(defun send-something1-time (loc socket desc stream str)
  (if timedbg
      (let ((univt1 (get-universal-time))
	    (realt1 (get-internal-real-time))
	    (runt1  (get-internal-run-time))
	    (lt1    (getms)))
	(let ((ret (send-something1 loc socket desc stream str)))
	  (let ((univt2 (get-universal-time))
		(realt2 (get-internal-real-time))
		(runt2  (get-internal-run-time))
		(lt2    (getms)))
	    (print-eml-loc loc (format nil "*SOMETHING-1* universal: ~A s; real: ~A ms; run: ~A ms; local: ~A ns"
				       (- univt2 univt1)
				       (- realt2 realt1)
				       (- runt2  runt1)
				       (- lt2    lt1)))
...
(defun send-something2 (loc socket desc stream str)
  ...
  (declare (ignore desc))
  ;;(print-eml-loc loc (format nil "sending something: ~A" str))
  (write str :stream stream)
  ;;(write smsg :stream stream)
...
(defun send-something2-time (loc socket desc stream str)
  (if timedbg
      (let ((univt1 (get-universal-time))
	    (realt1 (get-internal-real-time))
	    (runt1  (get-internal-run-time))
	    (lt1    (getms)))
	(let ((ret (send-something2 loc socket desc stream str)))
	  (let ((univt2 (get-universal-time))
		(realt2 (get-internal-real-time))
		(runt2  (get-internal-run-time))
		(lt2    (getms)))
	    (print-eml-loc loc (format nil "*SOMETHING-2* universal: ~A s; real: ~A ms; run: ~A ms; local: ~A ns"
				       (- univt2 univt1)
				       (- realt2 realt1)
				       (- runt2  runt1)
				       (- lt2    lt1)))
...
(defun send-something (loc socket desc stream str)
  (if new-send-receive
      (send-something2-time loc socket desc stream str)
      (send-something1-time loc socket desc stream str)
      )
  )
...
(defun send-public-key1-xxx (loc socket desc stream)
...
(defun receive-public-key1 (loc socket desc stream)
  (declare (ignore loc))
  (declare (ignore socket))
  (declare (ignore desc))
  (declare (ignore stream))
...
(defun receive-public-key2 (loc socket desc stream)
  (gen-receive loc socket desc stream 0)
  )

(defun receive-public-key (loc socket desc stream)
  (if new-send-receive
      (receive-public-key2 loc socket desc stream)
      (receive-public-key1 loc socket desc stream)
      )
  )
...
(defun connect-to (loc port gt)
  ...
  (if gt
	...
	(print-eml-loc loc (format nil "connected to (~A,~A)" h p))
	;; send port number to host using socket
	(send-port-number loc port socket desc stream)
	(print-eml-loc loc (format nil "sent port number ~A" port))
	;; if 'auth' is true 'loc' also send public key
	(if auth
	    (progn
	      (send-public-key loc socket desc stream)
	      (print-eml-loc loc (format nil "sent key ~A" (mk-public-key)))
	      (let ((key (receive-public-key loc socket desc stream)))
		(print-eml-loc loc (format nil "received key ~A" key))
		(setf (nfo-key nfo) key)
		)
	      )
	    )
...
(defun update-nfos-with-socket (port key socket nfos)
  (if nfos
      (let ((nfo (car nfos)))
	(multiple-value-bind (updated rest)
	    (update-nfos-with-socket port key socket (cdr nfos))
...
(defun wait-for-connections (loc server connected lt)
  ...
  (if lt
	...
	(let* ((fd   (socket-to-fd socket))
	  ...
	  (print-eml-loc loc (format nil "received port: ~A" port))
	  (let ((key (if auth (receive-public-key loc socket fd strm) nil)))
	    (if auth
		(progn
		  (print-eml-loc loc (format nil "reveived key: ~A" key))
		  (send-public-key loc socket fd strm)
		  (print-eml-loc loc (format nil "sent key ~A" (mk-public-key)))
		  ))
	    (multiple-value-bind (updated restlt)
		(update-nfos-with-socket port key socket lt)
...
(defun receive-one-message-len-parse2 (loc socket desc stream)
  ;;(gen-receive loc socket desc stream 0)
...
(defun receive-one-message-len-parse (b loc socket desc stream)
  (if new-send-receive
      (receive-one-message-len-parse2 loc socket desc stream)
...
;;   ;;(print-eml-loc loc (format nil "marshaling: ~A" msg))
;;   ;;(let ((x (ms:marshal msg)))
;;   ;;(print-eml-loc loc (format nil "marshaled"))
;;   (send-something loc socket desc stream msg)
;;   ;;    )
...
;; (defun send-message-time (loc socket desc stream msg delay)
;;   (let ((univt1 (get-universal-time))
;; 	(realt1 (get-internal-real-time))
;; 	(runt1  (get-internal-run-time))
;; 	(lt1    (getms)))
;;     (let ((ret (send-message loc socket desc stream msg delay)))
;;       (let ((univt2 (get-universal-time))
;; 	    (realt2 (get-internal-real-time))
;; 	    (runt2  (get-internal-run-time))
;; 	    (lt2    (getms)))
;; 	(print-eml-loc loc (format nil "*SEND-MSG* universal: ~A s; real: ~A ms; run: ~A ms; local: ~A ns"
;; 				   (- univt2 univt1)
;; 				   (- realt2 realt1)
;; 				   (- runt2  runt1)
;; 				   (- lt2    lt1)))
...
(defun send-length-and-message-dmh-time (loc dest socket desc stream str delay)
  (if timedbg
      (let ((univt1 (get-universal-time))
	    (realt1 (get-internal-real-time))
	    (runt1  (get-internal-run-time))
	    (lt1    (getms)))
	(let ((ret (send-length-and-message-dmh loc dest socket desc stream str delay)))
	  (let ((univt2 (get-universal-time))
		(realt2 (get-internal-real-time))
		(runt2  (get-internal-run-time))
		(lt2    (getms)))
	    (print-eml-loc loc (format nil "*SENDING* (~A) universal: ~A s; real: ~A ms; run: ~A ms; local: ~A ns"
				       delay
				       (- univt2 univt1)
				       (- realt2 realt1)
				       (- runt2  runt1)
...
(defun send-length-and-message (loc dest socket desc stream str delay)
  (handler-case
      (if dmh
	  (send-length-and-message-dmh-time loc dest socket desc stream str delay)
	  (send-length-and-message-threads  loc dest socket desc stream str delay)
	  )
    (sb-bsd-sockets:socket-error (c)
      (describe c)
      (print-eml-loc loc (format nil "send-length-and-message socket error: ~A" c))
      (print-eml-loc loc "closing socket")
      (handler-case
	  (progn (sb-bsd-sockets:socket-close socket)
		 (print-eml-loc loc "closed socket"))
	(error (c)
	  (print-eml-loc loc (format nil "error while trying to close socket: ~A" c))
	  (print-eml-loc loc "ignoring errors")
	  )
	)
      )
    (error (c)
...
(defun send-length-and-message-format1-T (loc dest socket desc stream info delay)
  ;; we use format to serialize/marshal
  (let ((s (format nil "~S" info)))
    ;;(let ((s (ms:marshal info)))
    (send-length-and-message loc dest socket desc stream s delay)))

(defun send-length-and-message-format1-T-time (loc dest socket desc stream info delay)
  (if timedbg
      (let ((univt1 (get-universal-time))
	    (realt1 (get-internal-real-time))
	    (runt1  (get-internal-run-time))
	    (lt1    (getms)))
	(let ((ret (send-length-and-message-format1-T loc dest socket desc stream info delay)))
	  (let ((univt2 (get-universal-time))
		(realt2 (get-internal-real-time))
		(runt2  (get-internal-run-time))
		(lt2    (getms)))
	    (print-eml-loc loc (format nil "*SENDING-1-T* universal: ~A s; real: ~A ms; run: ~A ms; local: ~A ns"
				       (- univt2 univt1)
				       (- realt2 realt1)
				       (- runt2  runt1)
				       (- lt2    lt1)))
	    ret)))
      (send-length-and-message-format1-T loc dest socket desc stream info delay)
      ))

;; ----
;; -- The Boolean b controls whether or not we want to convert the info
;; -- into a string.  If b is true then we do convert the info into a
;; -- string.
;; --
(defun send-length-and-message-format1 (b loc dest socket desc stream info delay)
  (if b
      (send-length-and-message-format1-T-time loc dest socket desc stream info delay)
...
(defun send-length-and-message-format2 (loc tosign dest socket desc stream info delay)
  (if (and delay (> delay 0))
      ;; If delay > 0 and the delayed message handler is active then
      ;; we have to send a message to the handler instead of the destination.
      ;; (This is done by send-output-dmh)
      ;; The content of the message should be (delay,dest,msg) instead of just msg.
      (let* ((msg (mk-directed-message delay dest info)))
	;;(send-message-time loc socket desc stream msg delay)
	(send-something loc socket desc stream msg))
	;; (print-eml-loc loc (format nil "new thread is going to send the delayed message (~A ms)" delay))
	;; (sb-thread:make-thread
	;;  (lambda ()
	...
	;;  :name "sender")
	;; )
      ;;(send-message-time loc socket desc stream info delay)
      (if (and tosign auth)
	  (let* (
		 (digest (mk-digest info))
		 (sig    (mk-sig loc digest))
		 (sigrs  (get-sig-rs sig))
		 (smsg   (cons loc (cons sigrs info)))
		 )
	    (print-eml-loc loc (format nil "signed message: ~A" smsg))
	    (send-something loc socket desc stream smsg))
	  (send-something loc socket desc stream info))
      )
  )

(defun send-length-and-message-format2-time (loc tosign dest socket desc stream info delay)
  (if timedbg
      (progn
	(print-eml-loc loc (format nil "(1) ~A" (local-time:now)))
	(let ((univt1 (get-universal-time))
	      (realt1 (get-internal-real-time))
	      (runt1  (get-internal-run-time))
	      (lt1    (getms)))
	  (let ((ret (send-length-and-message-format2 loc tosign dest socket desc stream info delay)))
	    (let ((univt2 (get-universal-time))
		  (realt2 (get-internal-real-time))
		  (runt2  (get-internal-run-time))
		  (lt2    (getms)))
	      (print-eml-loc loc (format nil "(2) ~A" (local-time:now)))
	      (print-eml-loc loc (format nil "*SENDING-2* (~A) universal: ~A s; real: ~A ms; run: ~A ms; local: ~A ns"
					 delay
					 (- univt2 univt1)
					 (- realt2 realt1)
					 (- runt2  runt1)
					 (- lt2    lt1)))
	      (print-eml-loc loc (format nil "(3) ~A" (local-time:now)))
	      ret))))
      (send-length-and-message-format2 loc tosign dest socket desc stream info delay)
      )
  )
...
;; -- The Boolean b controls whether or not we want to convert the info
;; -- into a string.  If b is true then we do convert the info into a
;; -- string.  This is only used when new-send-receive is turned off,
;; -- i.e., when not using the write function but the socket send
;; -- function.
;; --
;; -- The tosign Boolean indicates if the message has to be signed.
;; -- Sometimes we don't want to sign messages, for example when messages
;; -- are sent by DMHs, they are already signed, so we don't want to sign
;; -- them again.
;; --
;; -- Currently, signing is only available when new-send-receive is turned on.
;; --
(defun send-length-and-message-format (b loc tosign dest socket desc stream info delay)
  ;;(print-eml-loc loc (format nil "sending ~A to ~A" info dest))
  ;;  (if (string<= "client" dest)
  ;;      (send-length-and-message-format1 b loc socket desc stream info delay)
  (if new-send-receive
      (send-length-and-message-format2-time loc tosign dest socket desc stream info delay)
      (send-length-and-message-format1 b loc dest socket desc stream info delay)
      )
  ;;    )
  ;;(print-eml-loc loc (format nil "sent message to ~A" dest))
  )
...
(defun handle-new-connection (loc outfd stream socket)
  ...
  (let* (
	 ...
	 (b      nil) ;; meaning that there's no need to convert the message to a string because we're just forwarding it
	 (sock   nil)
	 (delay  nil)
	 (tosign nil)
	 )
    ...
    ;;(print-eml-loc loc "handling...")
    (send-length-and-message-format b loc tosign loc sock outfd stream str delay)
    ;;(print-eml-loc loc "handled...")
...
(defun keep-listening-for-inputs (loc outfd stream server)
  ...
  (let ((socket (sb-bsd-sockets:socket-accept server)))
    (sb-thread:make-thread
     (lambda ()
       (progn ;; new process
	 (print-eml-loc loc (format nil "handling new connection (~A)" (local-time:now)))
...
(defun keep-listening-for-inputs-xxxxxxxxx (loc outfd stream server)
  ;; Listen for incoming connections
  (let ((socket (sb-bsd-sockets:socket-accept server))
	(n (sb-posix:fork)))
    (if (= n 0)
...
(defun add-fd-to-nfos-with-key (nfos id fd key)
  (let ((nfo (mk-nfo-from-fd id fd)))
    (setf (nfo-key nfo) key)
    (cons nfo nfos)
    )
  )
...
(defun boot-up (ident conf-file spec)
  ...
  (let* (
    ...
    (if auth
	(progn (setq pbkey pbkey-test)
	       (setq prkey prkey-test)
	       (setq pbkey-p dsa-p-test)
	       (setq pbkey-q dsa-q-test)
	       (setq pbkey-g dsa-g-test)
	       (setq pbkey-y dsa-y-test)
	       )
	)
    (print-eml-loc loc (format nil "public key is ~A" pbkey))
    ;;(print-eml-loc loc (format nil "~A" (ironclad:dsa-key-p pbkey)))
	...
	(let ((new-nfos (add-fd-to-nfos-with-key nfos nil infd (mk-public-key))))
	  ;; I was trying to use posix-fork instead of sb-posix:fork,
	  ;; but I'm not sure what it does!
	  (sb-thread:make-thread
	   (lambda ()
	     (progn ;; new process
	       (print-eml-loc loc "listener thread, listening for inputs")
	  ...
	  (progn ;; parent process
	    (print-eml-loc loc (format nil "running process"))
...
(defun boot-up-xxxxxx (ident conf-file spec)
  (print-eml-loc ident "reading configuration file")
...
(defun get-delay-of-intransit-message (dmsg) (car dmsg))
(defun get-loc-of-intransit-message   (dmsg) (car (cdr dmsg)))
(defun get-msg-of-intransit-message   (dmsg) (cdr (cdr dmsg)))
...
(defun run-hdf-time (loc program msg)
  (if timedbg
      (let ((univt1 (get-universal-time))
	    ...
	    (runt1  (get-internal-run-time))
	    (lt1    (getms)))
	;;(print-eml-loc loc (format nil "running process on ~A" msg))
	(let ((ret (run-hdf loc program msg)))
	  (let ((univt2 (get-universal-time))
		(realt2 (get-internal-real-time))
		(runt2  (get-internal-run-time))
		(lt2    (getms)))
	    (print-eml-loc loc (format nil "*RUNNING* universal: ~A s; real: ~A ms; run: ~A ms; local: ~A ns"
				       (- univt2 univt1)
				       (- realt2 realt1)
				       (- runt2  runt1)
				       (- lt2    lt1)))
	    ret)))
      (run-hdf loc program msg)
      ))

(defun nuprl-list-to-lisp (lst)
  (if (consp lst)
      (cons (car lst) (nuprl-list-to-lisp (cdr lst)))
      nil
      )
  )
...
(defun lisp-list-to-nuprl (lst)
  (if lst
      (cons (car lst) (lisp-list-to-nuprl (cdr lst)))
      nil
      )
  )
  ...
  (let ((delay (get-delay-of-intransit-message output))
	...
	(let ((nfo (if (> delay 0) outnfo (find-nfo-with-id loc to nfos))))
	  (let (
		...
		(tosign t) ;; true because to send some output, the process has to be part of the running system
		)
	    (send-length-and-message-format t loc tosign dest socket desc stream msg delay)
	    nil
	    )
	  )
	)
...
(defun send-output (loc outnfo nfos output)
  ;;(print-eml-loc loc (format nil "sending: ~A" output))
  (if dmh
...
(defun run-on-messages (loc program msgs outnfo nfos)
  (if (and program msgs)
      (let ((msg  (car msgs))
	    (rest (cdr msgs)))
	(let* ((pair        (run-hdf-time loc program (mk-arg msg)))
	       (new-program (car pair))
	       (outputs     (intransit-messages-to-list (cdr pair))))
	  ;; TO COMMENT 2
...
(defun run-distributed-program-loop (loc outnfo nfos program)
  ...
  (let* ((msgs (receive-messages loc nfos)))
    ...
    ;;(print-eml-loc loc (format nil "messages: ~A" msgs))
    ...
    (let ((new-program (run-on-messages loc program msgs outnfo nfos)))
      (run-distributed-program-loop loc outnfo nfos new-program)
      )
    )
  )
...
(defun run-distributed-program-loop-dummy (loc nfos stream n)
  ...
  (let* ((msgs (receive-messages loc nfos)))
    ...
    (print-eml-loc loc (format nil "universal: ~A s" (get-universal-time)))
    (print-eml-loc loc (format nil "real: ~A ms" (get-internal-real-time)))
    (print-eml-loc loc (format nil "run: ~A ms" (get-internal-run-time)))
    (print-eml-loc loc (format nil "local: ~A ms" (local-time:now)))
    ;;)
    (when stream
...
(defun xxxx-run-distributed-program (ident config spec)
  (multiple-value-bind (loc member nfos program)
    ...
    (print-eml-loc loc (format nil "system member? ~A" member))
	...
	(if program
	    (progn
	      ...
	      (run-distributed-program-loop loc outnfo new-nfos program)
	      )
	    (progn
...
(defun verify-message (loc msg peer-loc peer-pbkey)
  (if (and auth peer-loc)
      (let (
	    (msg-loc  (car msg))
	    (msg-sig  (car (cdr msg)))
	    (msg-info (cdr (cdr msg)))
	    )
	(print-eml-loc loc "checking location")
	(if (string= peer-loc msg-loc)
	    (let (
		  (digest (mk-digest msg-info))
		  (key (mk-dsa-public-key peer-pbkey))
		  (sig (mk-dsa-sig msg-sig))
		  )
	      (print-eml-loc loc (format nil "checking signature ~A (~A) using key ~A (~A)" msg-sig sig peer-pbkey key))
	      (if (ironclad:verify-signature key digest sig)
		  (progn
		    (print-eml-loc loc "signature checks out")
		    msg-info
		    )
		  (progn
		    (print-eml-loc loc (format nil "couldn't verify signature"))
		    (error "bad signature")
		    )
		  )
	      )
	    (progn
	      (print-eml-loc loc (format nil "wrong location for ~A, received ~A" peer-loc msg-loc))
	      (error "wrong location")
	      )
	    )
	)
      ;; otherwise the message comes from the server socket handler
      ;; and does not need to be verified because it has not been
      ;; signed
      msg
      )
  )

(defun set-handlers (loc outnfo nfos program allnfos)
  (if nfos
      ;; we create the handlers
      (let* (
	     (nfo    (car nfos))
	...
	(sb-sys:with-fd-handler
	    (fd
		 ...
		 (declare (ignore x)) ;; x should be fd
		 (handler-case
		     (let ((msg0 (receive-one-message-len-parse t loc socket fd strm)))
		       ;;
		       ;; if peer is nil then that means that fd is
		       ;; connected to the server socket that keeps on
		       ;; listening for new connections.  In that
		       ;; case, we don't need to unsign because the
		       ;; thread that runs the server socket does not
		       ;; sign anything.
		       ;;
		       ;; TO COMMENT
		       (print-eml-loc loc (format nil "received inputs from ~A, its key is ~A" peer key))
		       ;;(print-eml-loc loc (format nil "received ~A ~A" msg (listp msg)))
		       (let ((msg (verify-message loc msg0 peer key)))
			 (setf nuprl-msgs
			       ;; Why would msgs be a list??
			       (append (if (and nil (listp msg)) msg (cons msg nil))
				       nuprl-msgs))
			 )
		       )
		   (error (c)
	  ...
	  (set-handlers loc outnfo (cdr nfos) program allnfos)
	  )
	)
      (if program
	  (progn
	    (print-eml-loc loc "running process")
	    (run-distributed-program-loop loc outnfo allnfos program)
	    )
	  (progn
	    (print-eml-loc loc "no program to run")
	    (let ((stream (if eml-debug
  ...
  (sb-thread:make-thread
   (lambda ()
     (let ((delay (get-delay-of-intransit-message output))
	   (to    (get-loc-of-intransit-message   output))
	   (msg   (get-msg-of-intransit-message   output)))
       ;;(print-eml-loc loc (format nil "sleeping (~A)..." output))
       (sleep (float (/ delay 1000)))
       ;;(print-eml-loc loc "slept...")
       (let ((nfo (if (string= loc to) outnfo (find-nfo-with-id loc to nfos))))
	 (let (
	       (socket (nfo-socket nfo))
	       (desc   (nfo-desc   nfo))
	       ...
	       (dest   (nfo-id     nfo))
	       (newdelay nil)
	       (tosign t) ;; DMHs have to sign messages because send-length-and-message-format2 doesn't do it
	       )
	   (send-length-and-message-format nil loc tosign dest socket desc stream msg newdelay)
	   )
	 )))
   :name "delayed-message-handler-sender")
  )
	  ...
	  #'(lambda (x)
		  ...
		  (let ((msgs (receive-one-message-len-parse t loc nil infd (fd2stream infd))))
		    ...
		    (setf delayed-nuprl-msgs
			  ;; Why would msgs be a list??
			  (append (if (and nil (listp msgs)) msgs (cons msgs nil))
				  delayed-nuprl-msgs))
		    )
		(error (c)
...
(defun delayed-message-handler-xxxxxxxxxxxxxx (loc infd outnfo nfos)
  (let ((n (sb-posix:fork)))
    (if (= n 0)
	(sb-sys:with-fd-handler
	    (infd
...
(defun run-distributed-program-dmh (ident config spec)
  (multiple-value-bind (loc member nfos program)
      (boot-up ident config spec)
    ;;
    ;; nfos already contains one end of the pipe linked to the server
    ;; socket that's listening for more connections.
    ;;
    (print-eml-loc loc (format nil "system member? ~A" member))
    ;;
	...
	(sb-posix:pipe)
	  ...
	  (sb-posix:pipe)
	(let ((new-nfos        (add-fd-to-nfos-with-key nfos loc in-fd-proc (mk-public-key)))
	  ...
	  (set-handlers loc out-nfo-proc new-nfos program new-nfos)
	  )
	)
      )
    )
...
(defun run-distributed-program-threads (ident config spec)
  (multiple-value-bind (loc member nfos program)
	...
	(set-handlers loc outnfo new-nfos program new-nfos)
	)
      )
    )
  )
...
(defun mk-msg (hdr val)
  (cons hdr val)
  )

(defun mk-signed-msg (sender sign hdr val)
  (cons sender (cons sign (mk-msg hdr val)))
  )

(defun mk-propose-msg (n)
  (let ((prp (cons n n))
	(hdr (cons "propose" nil)))
    (mk-msg hdr prp)
    )
  )
...
(defun mk-swap (n protocol)
  (let ((prp (cons n (cons protocol nil)))
	(hdr (cons "swap" nil)))
    (mk-msg hdr prp)
    )
  )
...
(defun run-sender-loop (fmkmsg loc member dest n m socket desc stream)
  (if (> n m)
      (print-eml-loc loc "done")
      (let* (
	     (msg   (funcall fmkmsg n))
	     (delay nil)
	     (tosign member)
	     )
	;; TO COMMENT 2
	...
	;; sleeps for 1ms
	(sleep 0.001)
	(run-sender-loop fmkmsg loc tosign dest (+ n 1) m socket desc stream)
	)
      )
  )

(defun run-sender (fmkmsg sndr dest n m conf-file)
  ;; sndr connects to dest
  (let* (
	 (socket    (new-socket))
	 (desc      (socket-to-fd socket))
	 (stream    (socket-to-stream socket))
	 (conf      (parse-conf-file conf-file))
	 (loc-dest  (get-id-in-groups dest (conf-groups conf)))
	 (ip-dest   (location-host loc-dest))
	 (port-dest (location-port loc-dest))
	 (groups    (conf-groups conf))
	 (group     (find-group-id sndr groups))
	 (member    (group-member group))
	 )
    ;; first we connect to id
    (print-eml-loc sndr (format nil "system member? ~A" member))
    (connect sndr socket ip-dest port-dest)
    (print-eml-loc sndr (format nil "connected to ~A" dest))
    ;; then we send proposals from n to m
    (run-sender-loop fmkmsg sndr member dest n m socket desc stream)
    )
  )
...
;; sender
(defun test-propose (n m conf-file) (run-sender #'mk-propose-msg "client1" "ldr2" n m conf-file))
(defun test-bcast   (n m conf-file) (run-sender #'mk-bcast-msg   "client1" "rep2" n m conf-file))
(defun test-swap    (n conf-file)   (run-sender #'mk-swap-paxos  "client1" "rep2" n n conf-file))
(defun test-swap23  (n conf-file)   (run-sender #'mk-swap-23     "client1" "rep2" n n conf-file))
(defun test-dummy   (n conf-file)   (run-sender #'mk-dummy-msg   "client1" "ldr2" n n conf-file))
...
(defun tst-client1 ()
  (sb-thread:make-thread
   (lambda () (run-distributed-program "client1" conf-file main-spec))
   :name "client"))
;; sender
(defun propose (n m) (run-sender #'mk-propose-msg "client1" "ldr2" n m conf-file))
(defun bcast   (n m) (run-sender #'mk-bcast-msg   "client1" "rep2" n m conf-file))
(defun swap    (n)   (run-sender #'mk-swap-paxos  "client1" "rep2" n n conf-file))
(defun swap23  (n)   (run-sender #'mk-swap-23     "client1" "rep2" n n conf-file))
(defun dummy   (n)   (run-sender #'mk-dummy-msg   "client1" "ldr2" n n conf-file))
...
(defun dest (pair)
  (if (consp pair)
	...
	(car pair) ;; this is the new protocol
	)
      (error "dest:length")
      )
  )
	  ...
	  (if (= delay 0)
		...
		(if prg
		    (let ((pair (run-hdf "xxx" prg (mk-arg msg))))
		      ...
		      (let ((nprg  (car pair))
			    (nmsgs (intransit-messages-to-list (cdr pair))))
