...
(* 'true' to allow free variables in terms *)
val with_free_vars = true
...
fun get_free_vars t = VARS.listItems (free_vars t)
	    ...
	    then let val tv = dest_variable term
		 in case lookup_clos e tv of
			SOME c => (*if is_simple_closure c
		      ...
		      | NONE =>
			if with_free_vars
			then let val termref = ref (term, false)
			     in ENV (MAP.insert (env, v, (v,b,termref)))
			     end
			else raise Fail ("add2env_one(" ^ tv ^ ")")
		 end
	    else if is_ct term
...
fun parameters_of_term (TERM (((_, _), params), _)) = params
  ...
  | parameters_of_term (CLO_TERM clos) = raise Fail "parameters_of_term(CLO_TERM)"
...
fun mk_nuprl_msg_interface_constraint_term class headers f =
...
fun mk_nuprl_verify_class_term                 term  = mk_nuprl_simple_term "verify-class"                  [term]
...
fun mk_nuprl_verify_class_program_term         term  = mk_nuprl_simple_term "verify-class-program"          [term]
...
fun mk_nuprl_make_authentic_msg_term     hdr    value  = mk_nuprl_simple_term "make-Authentic-Msg"     [hdr,    value]
...
fun mk_nuprl_base_process_class_term          e1    e2    e3    = mk_nuprl_simple_term "base-process-class"          [e1,    e2,    e3]
fun mk_nuprl_base_process_program_term        e1    e2    e3    = mk_nuprl_simple_term "base-process-class-program"  [e1,    e2,    e3]
fun mk_nuprl_eo_msgs_interface_delivered_term eo    class f     = mk_nuprl_simple_term "eo-msgs-interface-delivered" [eo,    class, f]
...
fun mk_nuprl_eo_msg_interface_constraint_term eo   class headers f    = mk_nuprl_simple_term "eo-msg-interface-constraint" [eo,   class, headers, f]
...
fun mk_nuprl_state_class5_term          init tr1 term1 tr2 term2 tr3 term3 tr4 term4 tr5 term5 = mk_nuprl_simple_term "state-class5"          [init, tr1, term1, tr2, term2, tr3, term3, tr4, term4, tr5, term5]
fun mk_nuprl_state_class5_program_term  init tr1 term1 tr2 term2 tr3 term3 tr4 term4 tr5 term5 = mk_nuprl_simple_term "state-class5-program"  [init, tr1, term1, tr2, term2, tr3, term3, tr4, term4, tr5, term5]
...
fun unfold_all (lib as {abs, tof}) (term as TERM (operator, bterms)) =
  ...
  | unfold_all lib (term as CBA_TERM (a, x, f)) =
    if false
    then CBA_TERM (unfold_all_rterm lib a,
		   x,
		   unfold_all_rterm lib f)
    else unfold_all lib (TERM ((("callbyvalueall",dtag),[]), [B_TERM ([],a), B_TERM ([x],f)]))
...
fun nuprl2lisp2_term check funs ind libop sub term =
	...
	in if is_nuprl_lambda_term f
	   then let val (r,g) = dest_lambda 0 f
		...
		in (funs',
		    "(fix \"" ^ v ^ "\" (lambda (" (* fix's 1st argument is a string---its name *)
		    ^ v
		    ^ ")"
		    ^ (if b then "" else "\n(declare (ignore " ^ v ^ "))")
		    ^ "\n"
	...
	let val vx    = dest_nuprl_var x
	    val v     = nuprl2lisp_var x
	    val sub'  = if false (* if false then call-by-name *)
	...
	let val (funs1,str1,vars1) = nuprl2lisp2_ref_term check funs  ind libop sub f
		...
		^ str1
		^ "\n"
		^ (if false  (* if false then call-by-name *)
		   then str2
		   else "(lambda () " ^ str2 ^ ")")
		^ ")"
	...
	in (funs3, str, uvars vars1 (uvars vars2 vars3))
      ...
      | TERM ((("let", tag), params), [B_TERM ([], a), B_TERM ([x], b)]) =>
	let val (funs1,str1,vars1) = nuprl2lisp2_ref_term check funs  ind libop sub a
	    val vx    = dest_nuprl_var x
	    val v     = nuprl2lisp_var x
	    val sub'  = if true (* if false then call-by-name *)
			then (*(#1 (SUB.remove (sub, vx))) handle _ => sub*)
			    SUB.insert (sub, vx, v)
			else SUB.insert (sub, vx, "(funcall " ^ v ^ ")")
	    val (funs2,str2,vars2) = nuprl2lisp2_ref_term check funs1 ind libop sub' b
		...
		^ v
		^ " "
		^ (if true (* if false then call-by-name *)
		   then str1
		   else "(lambda () " ^ str1 ^ ")")
		^ "))"
		^ "\n"
...
fun nuprl2lisp2 lib unfold term =
	...
	val prelude =
	    ...
	    ^ "(defstruct axiom)"
	    ^ "\n"
	    ^ "(defun fix (str f) (funcall f (lambda () (fix str f))))"
	    ^ "\n"
    in (*prelude ^*)
	foldr
...
fun dest_token n (TERM ((("token", _), [(t,k)]), [])) = (t,k)
  ...
  | dest_token n term = raise Fail ("dest_token(" ^ Int.toString n ^ "):" ^ opid_of_term term ^ "-" ^ toStringTerm term)
